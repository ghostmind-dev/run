...
export default async function misc(program: any) {
  ...
  ////////////////////////////////////////////////////////////////////////////////
  // INSTALL VS CODE EXTENSIONS FROM GITHUB RELEASES
  ////////////////////////////////////////////////////////////////////////////////

  misc
    .command('extensions')
    .description('install VS Code extensions from GitHub releases')
    .argument('<extension>', 'extension name to install')
    .action(async (extensionName: string) => {
      // Check if the extension name is supported
      if (extensionName !== 'hide-unhide') {
        console.log(`No extension with that name: ${extensionName}`);
        Deno.exit(1);
      }
      ...
      try {
        ...
        // GitHub API URL for the releases directory
        const repoUrl =
        ...
        // Filter for .vsix files and sort by name (assuming version numbers in filename)
        const vsixFiles = files
          .filter((file: any) => file.name.endsWith('.vsix'))
          .sort((a: any, b: any) => {
            // Extract version numbers from filename for proper sorting
            const aVersion = a.name.match(/(\d+(?:\.\d+)*)/)?.[0] || '0';
            const bVersion = b.name.match(/(\d+(?:\.\d+)*)/)?.[0] || '0';
        ...
        if (vsixFiles.length === 0) {
          console.log('No .vsix files found in the releases directory');
          Deno.exit(1);
        }

        const mostRecentFile = vsixFiles[0];
        console.log(`Found most recent extension: ${mostRecentFile.name}`);
        ...
        const downloadPath = `/tmp/${mostRecentFile.name}`;

        // Download the file
        console.log(`Downloading ${mostRecentFile.name}...`);
        const downloadResponse = await fetch(mostRecentFile.download_url);
        ...
        await Deno.writeFile(downloadPath, new Uint8Array(fileData));

        console.log(`Downloaded to: ${downloadPath}`);

        // Install the extension using cursor
        console.log('Installing extension with Cursor...');
        await $`cursor --install-extension ${downloadPath}`;
