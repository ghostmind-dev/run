...
const actArgmentsDefault = [
  ...
  {
    name: "--secret",
    value: `GCP_PROJECT_NAME=${Deno.env.get("GCP_PROJECT_NAME")}`,
  },
  {
    name: "--secret",
    value: `GCP_SERVICE_ACCOUNT_ADMIN=${Deno.env.get(
  ...
  {
    name: "--secret",
    value: `TERRAFORM_BUCKET_NAME=${Deno.env.get("TERRAFORM_BUCKET_NAME")}`,
  },
];
...
export async function actionSecretsSet() {
  ...
  const data = fs.readFileSync(resolve(".env"), "utf8");
